    ...
    const handleSelectAnswer = (questionId, answerId) => {
        ...
        setSelectedAnswers(prev => {
            ...
            if (question.isMultiple) {
                // Initialize as array if not present
                const currentArray = Array.isArray(currentSelection) ? currentSelection : [];

                if (currentArray.includes(answerId)) {
                    // Remove if already selected
                    const newArray = currentArray.filter(id => id !== answerId);
                    return { ...prev, [questionId]: newArray.length > 0 ? newArray : undefined };
                } else {
                    // Add to selection
                    return { ...prev, [questionId]: [...currentArray, answerId] };
                }
            } else {
                    ...
                    <div className="space-y-4">
                        {question.answers?.map(answer => {
                            const isSelected = question.isMultiple
                                ? Array.isArray(selectedAnswers[question.id]) && selectedAnswers[question.id].includes(answer.id)
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center gap-4">
                                        <div className={`w-6 h-6 ${question.isMultiple ? 'rounded-md' : 'rounded-full'} border-2 flex items-center justify-center flex-shrink-0 ${isSelected
